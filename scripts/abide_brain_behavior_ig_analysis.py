#!/usr/bin/env python3
"""
ABIDE ASD Brain-Behavior Analysis Using Integrated Gradient Features
...
# File paths
IG_CSV = '/oak/stanford/groups/menon/projects/mellache/2024_age_prediction/scripts/feature_attribution/abide_asd_features_IG_convnet_regressor_trained_on_hcp_dev_wIDS.csv'
...
def perform_bag_behavior_correlation(merged_df, ados_cols, output_dir):
    """Correlate Brain Age Gap with ADOS scores."""
    import os
    ...
    print(f"\n{'='*60}")
    print(f"BAG-ADOS Correlations: N={len(merged_df)}")
    print(f"{'='*60}")
    ...
    for ados_col in ados_cols:
        ...
        valid_mask = ~np.isnan(merged_df['BAG']) & ~np.isnan(merged_df[ados_col])
        bag_clean = merged_df.loc[valid_mask, 'BAG'].values
        ados_clean = merged_df.loc[valid_mask, ados_col].values
        
        if len(bag_clean) < 10:
            continue
        
        corr, pvalue = spss.spearmanr(bag_clean, ados_clean)
        print(f"  {ados_col}: ρ={corr:.3f}, p={pvalue:.4f}")
        ...
        if pvalue < 0.05:
            ...
            sns.set_style("white")
            sns.regplot(
                x=bag_clean, y=ados_clean, ci=None,
            ...
            ax.set_ylabel(f"{ados_col}", fontsize=15, labelpad=10)
            ax.set_title("ABIDE ASD BAG-Behavior", fontsize=14, pad=10)
            plt.tight_layout(pad=1.2)
            
            safe_name = ados_col.replace('/', '_').replace(' ', '_')
            png_path = os.path.join(output_dir, f'abide_BAG_{safe_name}_scatter.png')
            plt.savefig(png_path, format='png', dpi=300, bbox_inches='tight')
            plt.close(fig)
            print(f"    ✓ Saved: {png_path}")


def perform_brain_behavior_regression(features_df, ados_df, output_dir):
    """
    Perform PCA + LinearRegression to predict ADOS scores from IG features.
    """
    os.makedirs(output_dir, exist_ok=True)
    
    # Merge features with ADOS by subject_id
    ...
    print(f"\nMerged: {len(merged)} subjects with IG + ADOS data")
    
    # Get ADOS columns
    ados_cols = [col for col in ados_df.columns if 'ADOS' in col and col != 'subject_id']
    
    if not ados_cols:
        print("  ⚠︎ No ADOS columns found")
        return
    
    print(f"  ADOS measures: {len(ados_cols)}")
    
    # Extract ROI features
    roi_cols = [col for col in features_df.columns if col != 'subject_id'][:246]
    X = merged[roi_cols].values
    
    # Process each ADOS measure
    for ados_col in ados_cols:
        if ados_col not in merged.columns:
            continue
            
        Y = merged[ados_col].values
        ...
        print(f"\n  {ados_col}: ρ={corr:.3f}, p={pvalue:.4f}, N={len(Y_clean)}")
        ...
        if pvalue < 0.05:
            ...
            ax.spines[['right', 'top']].set_visible(False)
            ax.set_xlabel(f"Observed {ados_col}", fontsize=15, labelpad=10)
            ax.set_ylabel(f"Predicted {ados_col}", fontsize=15, labelpad=10)
            ax.set_title("ABIDE ASD Brain-Behavior", fontsize=14, pad=10)
            plt.tight_layout(pad=1.2)
            
            safe_name = ados_col.replace('/', '_').replace(' ', '_')
            png_path = os.path.join(output_dir, f'abide_{safe_name}_scatter.png')
            ai_path = os.path.join(output_dir, f'abide_{safe_name}_scatter.ai')
...
if __name__ == '__main__':
    print("="*80)
    print("ABIDE ASD Brain-Behavior Analysis (IG Features)")
    print("="*80)
