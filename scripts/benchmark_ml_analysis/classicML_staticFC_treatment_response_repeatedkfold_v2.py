...
def data_cleaning_adhd_fukui_original_sample(path_to_dataset):
    saved_data_path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/adhd_fukui_IIRV/fold_0.bin'
...
def get_features_labels(path_to_dataset, site):
    if site == 'adhd_fukui':
        data, labels = data_cleaning_adhd_fukui_original_sample(path_to_dataset)
    elif site == 'pd_MDS':
...
if __name__ == '__main__':
    ...
    model_ss = 'embarc_MDD_original_sample_repeatedkfold'

    output_path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/results/dnn/models/finetuning_embarc/ml_models_repeatedkfold/'
    ...
    all_accuracies_std = {i: [] for i in model_names}
    all_f1s_std = {i: [] for i in model_names}
    all_precisions_std = {i: [] for i in model_names}
    all_recalls_std = {i: [] for i in model_names}
    ...
    for name,model in models:
        ...
        for fold_number, (train_index,val_index) in enumerate(kf.split(data_features,labels)):
            x_train, x_val = data_features[train_index], data_features[val_index]
            y_train, y_val = labels[train_index], labels[val_index]
            #print('*** Classifier: ', name)
            print('** Evaluating: Fold {}'.format(fold_number))
            
            fname_model = output_path + 'model_' + name + '_' + model_ss + '_' + str(fold_number) + '.sav'
            ...
            classifreport_dict = classification_report(y_val, y_val_predicted,
                                                       output_dict=True)
            accuracy[fold_number] = classifreport_dict['accuracy'] * 100
            precision[fold_number] = classifreport_dict['macro avg']['precision'] * 100
            recall[fold_number] = classifreport_dict['macro avg']['recall'] * 100
            f1score[fold_number] = classifreport_dict['macro avg']['f1-score'] * 100
        ...
        all_accuracies_std[name].append(np.std(accuracy))
        all_f1s_std[name].append(np.std(f1score))
        all_precisions_std[name].append(np.std(precision))
        all_recalls_std[name].append(np.std(recall))
        print('*** Classifier: ', name)
        print(all_accuracies)
        print(all_f1s)
    print('Accuracy:',all_accuracies)
    print('F1-Score:',all_f1s)
    print('Precision:',all_precisions)
    print('Recall:',all_recalls)
    print('Accuracy SD:',all_accuracies_std)
    print('F1-Score SD:',all_f1s_std)
    print('Precision SD:',all_precisions_std)
    print('Recall SD:',all_recalls_std)
