...
def get_features_labels(path_to_dataset, site):
    if site == 'hcp':
        ...
        data, labels = data_cleaning_abide_asd(path_to_dataset)
    elif site == 'bsnip':
...
def perform_stability_analysis(path_to_dataset,site,sample_size,times_to_sample,imbalance_ratio,output_path,model_ss):
    result_dir = f"/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/scripts/dnn/benchmark_ml_analysis/stability_analysis/hcp_gender_results/" 
    ...
    np.savez(result_dir + f"stability_analysis_ml_methods_{site}_sample_size_{sample_size}_male_female_ratio_{imbalance_ratio}_1_sample_{times_to_sample}_times_results.npz",mean_accuracy = mean_acc,std_accuracy=std_acc,mean_f1=mean_f1,std_f1=std_f1)
 
    return mean_acc,std_acc,mean_f1,std_f1

if __name__ == '__main__':
    hcp_data = '/oak/stanford/groups/menon/deriveddata/public/hcp/restfmri/timeseries/group_level/brainnetome/normz/hcp_run-rfMRI_REST1_LR_brainnetome_mean_regMov-6param_wmcsf_dt1_bpf008-09_normz_246ROIs.pklz'
    ...
    output_path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/results/dnn/models/finetuning_hcp_gender/ml_models/'
    model_ss = 'hcp_gender'
    site = 'hcp'
    times_to_sample = 40
    for sample_size in [50,100,200,400]:
        for imbalance_ratio in [1,2,3,4]:
            accs,acc_stds,f1s,f1_stds = perform_stability_analysis(hcp_data,site,sample_size,times_to_sample,imbalance_ratio,output_path,model_ss)
            print(accs)
            print(acc_stds)
            print(f1s)
            print(f1_stds)
