...
def test_model_getVals(x_valid,y_valid,scaler,hyper_parameters, fname_model):
    ...
    with torch.no_grad():
        for images, labels in valid_loader:
            ...
            regressor.fit(embeddings,labels_numpy)
            outputs = regressor.predict(embeddings)
        return np.squeeze(scaler.inverse_transform(labels.cpu().numpy().reshape(-1,1))), np.squeeze(scaler.inverse_transform(outputs.reshape(-1,1)))
