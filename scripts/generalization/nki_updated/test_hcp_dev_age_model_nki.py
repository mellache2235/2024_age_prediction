...
for fold in range(5):
    ...
    path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/nki_age_cog_dev_wIDs/fold_%d.bin'%(fold)
    ...
    #genders_valid = datao.loc[datao['subject_id'].isin(id_valid),'gender']
    #genders_all.append(genders_valid)
    #corrs.append(valid_corr_coef)
    #pvals.append(valid_pval)
    #maes.append(valid_mae)
    #ids_all.append(id_valid)
 
    #actual, predicted = test_model_getVals(X_valid,Y_valid,sc,hyper_parameters,hcp_dev_model,intercept,slope)
...
#df = pd.DataFrame({'ID':ids_all,'BAG':np.squeeze(predicted_ages)-np.squeeze(actual_ages),'Gender':genders_all})
#print(df)
#np.savez("lin_model_params_nki_5folds.npz", coef=coef_list, intercept=intercept_list)
#pdb.set_trace()
...
ax.set_title("NKI-RS",fontsize=16)
plt.savefig('nki_brain_age_scatter_test_bias_correction.png',format='png')
