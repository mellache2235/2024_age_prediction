...
def test_model(x_valid,y_valid,scaler,hyper_parameters, fname_model):
    ...
    with torch.no_grad():
        for images, labels in valid_loader:
            ...
            #print(loss)
            actual = np.squeeze(scaler.inverse_transform(labels.cpu().numpy().reshape(-1,1)))
            predicted = np.squeeze(scaler.inverse_transform(outputs.cpu().numpy().reshape(-1,1))) 
            test_corr_coeff, pval = scipy.stats.pearsonr(actual,predicted)
...
for fold in range(5):
    ...
    path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/cmihbn_age_TD/fold_%d.bin'%(fold)
 
    X_train, X_valid, Y_train, Y_valid = load_finetune_dataset(path)    
    '''
    path_nki = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/nki_age_cog_dev/fold_%d.bin'%(fold)
    X_train_nki, X_valid_nki, Y_train_nki, Y_valid_nki = load_finetune_dataset(path_nki)
 
    sc = StandardScaler()
    sc.fit(Y_train_nki.reshape(-1,1))
    '''
...
ax.set_title("CMI-HBN TD",fontsize=16)
plt.savefig('cmihbn_td_brain_age_scatter_test_bias_correction.png',format='png')

pdb.set_trace()
'''
np.savez('cmi_td_predicted_ages',predicted=predicted_ages)
np.savez('cmi_td_actual_ages',actual=actual_ages)
pdb.set_trace()
