...
# run_cv5_trainval_fixed_v2.py
# ------------------------------------------------------------
# 5-fold run using TRAIN and VALIDATION per fold.
# Your .bin uses X_train / X_test / y_train / y_test where "test" is actually validation.
# This script aliases X_test->X_val and y_test->y_val automatically.
# - No extra split.
# - 100 epochs, batch size 32.
# - Bias fit on TRAIN only; compute BAG for TRAIN and VAL.
# - Assumes arrays are [N, T, C]; set TRANSPOSE_NTC=False if already [N, C, T].
# ------------------------------------------------------------
...
ALIASES = {
    ...
    # Treat "test" keys as validation if explicit val keys are absent
    'X_val':   ['X_val','x_val','X_valid','validX','X_validation','X_test','x_test'],
    'y_val':   ['y_val','Y_val','validY','y_valid','y_validation','y_test','Y_test'],
    'ids_train': ['ids_train','train_ids','subjects_train','sid_train'],
    'ids_val':   ['ids_val','valid_ids','subjects_val','sid_val','ids_test','test_ids','subjects_test','sid_test'],
}
...
def load_trainval_bin(path):
    ...
    if Xtr is None or ytr is None or Xva is None or yva is None:
        raise ValueError(f"{path}: expected TRAIN/VAL keys (e.g., X_train/y_train and X_val|X_test, y_val|y_test).")
    idtr = _get(d,'ids_train'); idva = _get(d,'ids_val')
    extra_keys = set(d.keys()) - set(sum(ALIASES.values(), []))
    extra = {k:v for k,v in d.items() if k in extra_keys}
