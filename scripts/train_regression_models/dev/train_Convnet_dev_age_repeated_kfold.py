...
def run_all_folds():
    ...
    for fold_number, (train_index,val_index) in enumerate(rskf.split(X,Y)):
            ...
            fname_model = result_dir + 'models/dev/repeated_kfold_part1/Convnet_regressor_dev_age_fold_%d.pt'%(fold_number)
            fname_figure = result_dir + 'figures/dev/repeated_kfold_part1/Convnet_regressor_dev_age_fold_%d'%(fold_number)
