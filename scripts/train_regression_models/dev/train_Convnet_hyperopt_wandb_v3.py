...
def train(model, dataloader_1,dataloader_2, criterion, optimizer, fname_model,val_loss_temp,device):
    ...
    return val_loss_temp

def evaluate(model, dataloader, criterion, fname_model,device):
    model = ConvNet_v2()
...
entity = 'mellache'
project = 'nested_cv_hcp_dev_age_model_v2'
...
def train_with_wandb(config=None):
    ...
    with wandb.init(config=config):
        ...
        for inner_train_idx, inner_val_idx in inner_cv.split(X_train,y_train):
            ...
            model = ConvNet_v2(config.dropout_rate).to(device)
            ...
            # Train the model
            fname_model = 'best_inner_fold_hcp_dev_age_model_model_v2.pt'
            val_loss_temp = 100000.0
            for epoch in range(config.epochs):
                print(f'Epoch {epoch + 1}/{config.epochs}')
                val_loss_temp = train(model, train_loader, val_loader,criterion, optimizer, fname_model,val_loss_temp,device)
            predicted, actual = evaluate(model, val_loader, criterion, fname_model,device)
            ...
            if val_loss_temp < best_inner_score:
                ...
                best_model_filename = f'best_model_inner_fold.pt'
                torch.save(model.state_dict(),best_model_filename)
                artifact = wandb.Artifact(f'best_model_inner_fold',type='model')
...
for fold in range(5):
    ...
    model = ConvNet_v2(best_hyperparams['dropout_rate']).to(device)
    ...
    fname_model_outer = f'best_outer_fold_{fold}_hcp_dev_model_v2.pt'
    print('Outer Fold Training!')
    test_loss_temp = 1000000.0
    for epoch in range(best_hyperparams['epochs']):
        test_loss_temp = train(model, train_loader, test_loader,criterion, optimizer, fname_model_outer,test_loss_temp,device)
    
    # Evaluate on outer test set
    predicted,actual = evaluate(model, test_loader, criterion,fname_model_outer,device)
    corr_coef,p = scipy.stats.pearsonr(x=np.squeeze(actual),y=np.squeeze(predicted))
