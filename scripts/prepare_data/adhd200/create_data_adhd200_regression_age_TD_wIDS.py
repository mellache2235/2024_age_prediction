...
ix = labels != 0
labels[ix] = 1
new_df = pd.DataFrame({'subject_id':ids,'data':list(df['data']),'mean_fd':mean_fds,'age':ages,'gender':genders,'site':sites,'label':labels,'trs':trs})
...
print(new_df_TD)
#pdb.set_trace()
new_df_TD = new_df_TD.reset_index()
X = np.asarray([np.asarray(i)[len(i)-174:] for i in new_df_TD["data"].values])
Y = np.asarray(new_df_TD['age'])
#print(Y)
#pdb.set_trace()
adhd_tr = new_df_TD["trs"].values
ids = new_df_TD['subject_id']
...
new_df_TD = new_df_TD.iloc[samples_to_keep,:]
new_df_TD = new_df_TD.reset_index()
X = np.asarray([np.asarray(i)[len(i)-174:] for i in new_df_TD["data"].values])
Y = np.asarray(new_df_TD['age'])
ids = new_df_TD['subject_id']
...
print(ids)
pdb.set_trace()
create_kfold_partitions(X,Y,ids,"/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/adhd200_regression_age_TD_wIDs/" , 256,64,5)
pdb.set_trace()
