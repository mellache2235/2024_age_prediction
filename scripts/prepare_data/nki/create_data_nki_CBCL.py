...
ids = np.asarray([i for ind,i in enumerate(datao['subject_id']) if ind not in new_indices])

frame = pd.DataFrame({'ID':ids,'data':list(data),'age':ages})
frame['ID'] = frame['ID'].astype('str')

cbcl_data = pd.read_csv('8100_CBCL_20191009.csv',skiprows=0)
cbcl_data['Anonymized ID'] = cbcl_data['Anonymized ID'].astype('str')
cbcl_data = cbcl_data.rename(columns={'Anonymized ID' : 'ID'})

new_frame = pd.merge(frame,cbcl_data,on='ID')
ocd_features = ['data',f"Can't get his/her mind off certain thoughts; obsessions",f"Fears he/she might think or do something bad",f"Feels he/she has to be perfect",f"Nervous  highstrung  or tense",f"Too fearful or anxious",f"Feels too guilty",f"Repeats certain acts over and over; compulsions",f"Strange behavior",f"Strange Ideas",f"Worries"]
#new_frame_ocd = new_frame.loc[:,[f"Can't get his/her mind off certain thoughts; obsessions",f"Fears he/she might think or do something bad",f"Feels he/she has to be perfect",f"Nervous  highstrung  or tense",f"Too fearful or anxious",f"Feels too guilty",f"Repeats certain acts over and over; compulsions",f"Strange behavior",f"Strange Ideas",f"Worries"]]
print(new_frame)
new_frame = new_frame.dropna()
new_frame = new_frame.reset_index()

OCS_2_items = [f"Can't get his/her mind off certain thoughts; obsessions", f"Repeats certain acts over and over; compulsions"]
OCS_6_items = [f"Can't get his/her mind off certain thoughts; obsessions", f"Repeats certain acts over and over; compulsions", f"Fears he/she might think or do something bad", f"Feels he/she has to be perfect", f"Nervous  highstrung  or tense", f"Too fearful or anxious"]
OCS_8_items = [f"Can't get his/her mind off certain thoughts; obsessions", f"Repeats certain acts over and over; compulsions", f"Fears he/she might think or do something bad", f"Feels he/she has to be perfect", f"Nervous  highstrung  or tense", f"Too fearful or anxious", f"Strange behavior",f"Strange Ideas",f"Worries",f"Feels too guilty"]
...
create_kfold_partitions(data,labels,'/oak/stanford/groups/menon/projects/mellache/2024_age_prediction/data/nki_ocd_vs_hc/',256,64)
