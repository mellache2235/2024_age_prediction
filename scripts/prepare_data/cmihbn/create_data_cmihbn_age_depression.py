...
'''
depression_scores = pd.read_csv('Cmihbn-ChildrensDepressionI_ParentReport_DATA_2024-11-04_1253.csv')
depression_scores = depression_scores.dropna(subset=['cdi_p_cdi2p_total_t'])
depression_scores = depression_scores.reset_index()
depression_scores['basic_demos_eid'] = depression_scores['basic_demos_eid'].astype('str')
depression_scores = depression_scores.rename(columns={'basic_demos_eid':'subject_id'})

new_frame = pd.merge(df,depression_scores,on='subject_id')
print(new_frame)
pdb.set_trace()
...
create_kfold_partitions(X,Y, ids,"/oak/stanford/groups/menon/projects/mellache/2024_age_prediction/data/cmihbn_age_TD/" , 256,64,5)
