...
'''
anxiety_scores = pd.read_csv('Cmihbn-StateTraitAnxietyInv_DATA_2024-11-01_1210_18+years.csv')
anxiety_scores = anxiety_scores.dropna(subset=['stai_stai_ta_t'])
anxiety_scores = anxiety_scores.reset_index()
anxiety_scores['basic_demos_eid'] = anxiety_scores['basic_demos_eid'].astype('str')
anxiety_scores = anxiety_scores.rename(columns={'basic_demos_eid':'subject_id'})

new_frame = pd.merge(df,anxiety_scores,on='subject_id')
print(new_frame)
pdb.set_trace()
