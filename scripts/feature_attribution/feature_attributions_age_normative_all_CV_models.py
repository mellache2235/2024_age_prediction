...
if __name__ == '__main__':
    ...
    #hy_data['Hyper/Impulsive'] = pd.to_numeric(hy_data['Hyper/Impulsive'], errors='coerce') 
    ...
    ids_total = np.concatenate((id_train,id_test))
    ...
    Offset = coefs[0] * actual_ages + intercepts[0]

    predicted_ages = ensemble_predictions - Offset     
    print(actual_ages)
    print(predicted_ages)
    ids_total = ids_total.astype('str')
    print(ids_total)
    print(hy_data['subject_id'])
    ids_total = pd.Series(ids_total)

    ins_all = np.asarray(ids_total.map(hy_data.set_index('subject_id')['Inattentive']))
    hys_all = np.asarray(ids_total.map(hy_data.set_index('subject_id')['Hyper/Impulsive']))
    hys_all = hys_all.astype(float)
    ins_all = ins_all.astype(float)
    sites_all = np.asarray(ids_total.map(hy_data.set_index('subject_id')['site']))

    ix = ins_all == -999.
    actual_ages = actual_ages[~ix]
    predicted_ages = predicted_ages[~ix]
    #hys_all = hys_all[~ix]
    ins_all = ins_all[~ix]
    sites_all = sites_all[~ix]
    ...
    #r,p = scipy.stats.pearsonr(bag,ins_all)
    
    min_index = np.argmin(ins_all)
    ins_all_new = np.delete(ins_all, min_index)
    actual_ages_new = np.delete(actual_ages, min_index)
    predicted_ages_new = np.delete(predicted_ages, min_index)
    bag = predicted_ages_new - actual_ages_new
    r,p = scipy.stats.pearsonr(bag,ins_all_new)
    print(r)
    print(p)
    r_squared = r ** 2
    fig,ax = plt.subplots()
    ...
    sns.regplot(x=bag, y=ins_all_new, ci=None,
    ...
    ax.set_ylabel("Inattention",fontsize=16)
    ax.set_title("ADHD-200 Peking",fontsize=16)
    plt.savefig('adhd200_Peking_BAG_IN_scatter.png',format='png') 
    pdb.set_trace()
    ...
    plt.savefig('adhd200_BAG_HY_NYU_only_scatter_test_bias_correction_single_model_prediction.png',format='png')
    ...
    ax.set_ylabel("Hyperactivity",fontsize=16)
    ax.set_title("ADHD200",fontsize=16)
    plt.savefig('adhd200_BAG_HY_Peking_only_scatter_test_bias_correction_single_model_prediction.png',format='png')
    pdb.set_trace()
    ...
    ax.set_title("ADHD200",fontsize=16)
    plt.savefig('adhd200_elastic_net_features_HY_Peking_only_scatter.png',format='png')
    #print(len(ids_all))
    #print(len(hys_all))
