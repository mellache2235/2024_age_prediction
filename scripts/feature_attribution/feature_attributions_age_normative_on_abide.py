...
if __name__ == '__main__':
    ...
    appended_data_asd = appended_data[ix]
    ix = appended_data_asd['age'] <= 21
    appended_data_asd_dev = appended_data_asd[ix]
    appended_data_asd_dev = appended_data_asd_dev.reset_index()
    ...
    for fold in range(5):
        ...
        '''
        mask = np.isin(appended_data_asd_dev['subject_id'],id_test)
         
        social_data_subset = appended_data_asd_dev[mask]
        ...
        scores = np.asarray(social_data_subset_order['ados_social'])
    ...
    social_all = np.asarray(ids_total.map(appended_data_asd_dev.set_index('subject_id')['ados_total']))

    ix = ~pd.isna(social_all)
    actual_ages = actual_ages[ix]
    predicted_ages = predicted_ages[ix]
    social_all = social_all[ix]

    bag = predicted_ages - actual_ages
    '''
    r,p = scipy.stats.pearsonr(bag,social_all)
    print(r)
    print(p)
    r_squared = r ** 2
    fig,ax = plt.subplots()
    ...
    sns.regplot(x=bag, y=social_all, ci=None,
    ...
    ax.set_ylabel("ADOS Total",fontsize=16)
    ax.set_title("ABIDE ASD",fontsize=16)
    plt.savefig('abide_asd_bag_ados_total_scatter.png',format='png')
    '''
    #pdb.set_trace()    
    #features_df = get_and_analyze_features(data_all, labels_all.flatten().astype('float64'),ids_all)
    #features_df.to_csv('abide_asd_features_IG_convnet_regressor_trained_on_hcp_dev_top_regions_wIDS.csv')
    features_df = pd.read_csv('abide_asd_features_IG_convnet_regressor_trained_on_hcp_dev_top_regions_wIDS.csv')
    ...
    plt.savefig('abide_asd_pca_loadings_ados_total.png')
    pdb.set_trace()
    '''
    ...
    ax.text(0.95, 0.05, f"$\mathit{{R}}^2$ = {r_squared:.3f}\n"f"{p_text}",
         ...
         fontsize=12)
    ax.spines[['right', 'top']].set_visible(False)
    ax.set_xlabel("Observed ADOS Communication",fontsize=16,labelpad=10)
    ax.set_ylabel("Predicted ADOS Communication",fontsize=16,labelpad=10)
    ax.set_title("Brain-Behavior Analysis: ABIDE",fontsize=16,pad=10)
    plt.tight_layout(pad=1.2)
    #plt.savefig('abide_asd_pca_regression_features_ados_total_scatter.png',format='png')
    pdf.FigureCanvas(fig).print_pdf('abide_asd_pca_regression_features_ados_comm_scatter.ai')


    #np.savez('predicted_abide_asd_ados_total_all_regions_dev_model',output=outputs)
    #np.savez('actual_abide_asd_ados_total_all_regions_dev_model',output=labels) 
    #print(len(ids_all))
    #print(len(hys_all))
