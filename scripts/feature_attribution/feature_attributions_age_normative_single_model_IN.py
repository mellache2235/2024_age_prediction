...
if __name__ == '__main__':
    ...
    id_total = np.concatenate((id_train,id_test))
    ...
    id_total = id_total.astype('str')
        
    X_total = reshapeData(X_total)
 
    mask = np.isin(hy_data['subject_id'],id_total)
         
    #hy_data_subset = hy_data[mask]
    #order = np.argsort([np.where(id_total == id_)[0][0] for id_ in hy_data_subset['subject_id']])
    ...
    #features_df, predicted_ages = get_and_analyze_features(X_total,Y_total,ids_all, sc)
    #features_df.to_csv('adhd200_adhd_features_all_sites_IG_convnet_regressor_trained_on_hcp_dev_top_regions_wIDS_single_model_predictions.csv')
    
    features_df = pd.read_csv('adhd200_adhd_features_all_sites_IG_convnet_regressor_trained_on_hcp_dev_top_regions_wIDS_single_model_predictions.csv')
    ...
    features_df['IN'] = np.asarray(features_df['subject_id'].map(hy_data.set_index('subject_id')['Inattentive']))
    features_df['IN'] = features_df['IN'].astype(float)
    ...
    ins_nyu = np.asarray(features_df.loc[features_df['site'] == 'NYU','IN'])
    ...
    frame = pd.DataFrame({'BAG':BAG,'IN':ins_nyu})
    print(frame)
    ...
    ax.set_title("Resilience-Behavior Analysis: ADHD-200 NYU",fontsize=16,pad=10)
    plt.tight_layout(pad=1.2)
    pdf.FigureCanvas(fig).print_pdf('adhd200_BAG_IN_NYU_only_pretty.ai')
    #plt.savefig('adhd200_BAG_IN_NYU_only_scatter_test_bias_correction_single_model_prediction.png',format='png')
    '''
    ...
    ins_peking = np.asarray(features_df.loc[features_df['site'] == 'Peking','IN'])
    #hys_all = hys_all[~pd.isna(hys_all)]
    #print(actual_subset.shape)
    #print(predicted_subset.shape)
    #print(hys_all.shape)
    ...
    frame = pd.DataFrame({'BAG':BAG,'IN':ins_peking})
    print(frame)
    corr, pvalue = spss.spearmanr(frame['BAG'], frame['IN'])
    print(corr)
    print(pvalue)
    ...
    ax.set_xlabel("Brain Age Gap",fontsize=16,labelpad=10)
    ax.set_ylabel("Inattention",fontsize=16,labelpad=10)
    ax.set_title("Resilience-Behavior Analysis: ADHD-200 Peking",fontsize=16,pad=10)
    #plt.savefig('adhd200_BAG_IN_Peking_only_scatter_test_bias_correction_single_model_prediction.png',format='png')
    plt.tight_layout(pad=1.2)
    pdf.FigureCanvas(fig).print_pdf('adhd200_BAG_IN_Peking_only_pretty.ai')
    '''
    #pdb.set_trace()
    ...
    X = features_df_peking.iloc[:,0:246]
    Y = np.asarray(features_df_peking.iloc[:,246])
    print(X)
    print(Y)
    pdb.set_trace()
    sc = StandardScaler()
    ...
    ax.spines[['right', 'top']].set_visible(False)
    ax.set_xlabel("Observed Inattention",fontsize=15,labelpad=10)
    ax.set_ylabel("Predicted Inattention",fontsize=15,labelpad=10)
    ax.set_title("Brain-Behavior Analysis: ADHD-200 NYU",fontsize=16,pad=10)
    plt.tight_layout(pad=1.2)
    #plt.savefig('adhd200_pca_regression_features_IN_Peking_only_scatter.png',format='png')
    #pdf.FigureCanvas(fig).print_pdf('adhd200_pca_regression_features_IN_NYU_only_scatter_pretty.ai')
    #print(len(ids_all))
    #print(len(hys_all))
