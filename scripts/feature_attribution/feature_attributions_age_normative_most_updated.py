...
def get_and_analyze_features(data_all, labels_all ,scaler):
...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/adhd200_regression_age_TD_wIDs/fold_%d.bin'%(fold)
        ...
        scores = np.asarray(hy_data_subset_order['Hyper/Impulsive'])
    ...
    features_df, predicted_ages = get_and_analyze_features(data_all, labels_all.flatten().astype('float64'), sc)
    ...
    features_df.drop(features_df.columns[50],axis=1, inplace=True)
    print(features_df)  
    #print(ids_all)
    #print(hys_all)
    X = features_df.iloc[:,0:50]
    Y = features_df.iloc[:,50]
