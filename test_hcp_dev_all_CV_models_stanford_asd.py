...
path = '/oak/stanford/groups/menon/projects/mellache/2021_foundation_model/data/imaging/for_dnn/stanford_autism_age_wIDS/fold_0.bin'
...
mean_per_subject_region = np.mean(X_total, axis=1, keepdims=True)
std_per_subject_region = np.std(X_total, axis=1, keepdims=True)
X_total = (X_total - mean_per_subject_region) / (std_per_subject_region + epsilon)
...
lin_params = np.load('/oak/stanford/groups/menon/projects/mellache/2024_age_prediction/scripts/generalization/asd_updated/lin_params_td_stanford.npz')
...
np.savez('actual_stanford_asd_ages_most_updated',actual=actual_ages)
np.savez('predicted_stanford_asd_ages_most_updated',predicted=predicted_ages)
pdb.set_trace()

mae = np.mean(np.abs(predicted_ages-actual_ages))
mean_bag = np.mean(predicted_ages-actual_ages)
print('MAE:',mae)
print('Mean BAG:',mean_bag)
pdb.set_trace()
...
ax.set_title("Stanford ASD Brain Age Prediction",fontsize=16,pad=10)
plt.tight_layout(pad=1.2)
#plt.savefig('stanford_asd_brain_age_scatter_ensemble_prediction_test_bias_correction_high_quality.png',format='png')
pdf.FigureCanvas(fig).print_pdf('stanford_asd_brain_age_scatter_ensemble_prediction_all_td_bias_correction_high_quality.ai')
pdb.set_trace()
